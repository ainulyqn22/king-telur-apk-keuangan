...
export default function MasterDataView({
  ...
  useEffect(() => {
    if (controller.error) showToast(controller.error, "error");
  ...
  return (
    <div className="grid lg:grid-cols-3 gap-6">
      <Section title="Supplier">
        ...
        />
        {controller.suppliers.map((v) => (
          <Row
            key={v.id}
            name={v.name}
            remove={() => {
              void run(
                () => controller.remove("suppliers", v.id),
      ...
      <Section title="Customer">
        ...
        />
        {controller.customers.map((v) => (
          <Row
            key={v.id}
            name={v.name}
            remove={() => {
              void run(
                () => controller.remove("customers", v.id),
      ...
      <Section title="Kategori Biaya">
        ...
        />
        {controller.categories.map((v) => (
          <Row
            key={v}
            name={v}
            remove={() => {
              void run(
                () => controller.removeCategory(v),
...
function Row({ name, remove }: { name: string; remove: () => void }) {
  return (
    <div className="flex justify-between border-t pt-2 text-sm">
      <span>{name}</span>
      <button onClick={remove} className="text-rose-600">
        Nonaktifkan
      </button>
    </div>
  );
